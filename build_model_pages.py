...
import sys
sys.path.insert(0, '.')
from parse_vw_specs import parse_vw_specs

# --- Load data -------------------------------------------------------------

with open("/home/claude/data.js", encoding="utf-8") as f:
    data_text = f.read()
DATA = json.loads(data_text[data_text.index("["):data_text.rindex("]") + 1])

import subprocess

with open("/home/claude/articles.js", encoding="utf-8") as f:
    art_text = f.read()
...
with open("vw_master_full.txt", encoding="utf-8") as f:
    VW_SPECS = parse_vw_specs(f.read())
...
HEADER = f"""<header>
  <div class="inner">
    <a href="carforyou.html" class="logo">{LOGO_SVG}</a>
    <nav>
      <a href="carforyou.html#compare">Priser</a>
...
def find_comparison_suggestion(maerke, model, type_, cheapest_price, all_grouped):
    """Suggests a model to compare against: same body type, closest price,
    preferring a different brand (a same-brand comparison is less useful
    than seeing how this model stacks up against the competition)."""
    candidates = []
    ...
    if not candidates:
        return None
    # Prefer different brand (True sorts after False when reversed, so flip)
...
def build_price_range_svg(variants):
    """
    An original, data-driven hero illustration per model page: shows where
    each variant's monthly price sits within the model's overall price
    range. Genuinely reflects the model's own real data -- not decorative,
    not a stock photo, no rights question.
    """
...
def build_model_page(maerke, model, records, all_grouped=None):
    ...
    variants = sorted(records, key=lambda r: r["ydelse_kr"])

    tags = [slugify(maerke), type_, "alle-maerker"]
    related = related_articles(tags)
    price_range_svg = build_price_range_svg(records)

    # Specs: only VW has real verified data right now.
    spec_html = ""
    matched_specs = [VW_SPECS[(model, r["variant"])] for r in records if (model, r["variant"]) in VW_SPECS]
    if matched_specs:
        s = matched_specs[0]
        boxes = []
        if s["type"] == "electric":
            ranges = [sp["raekkevidde_km"] for sp in matched_specs if sp["type"] == "electric"]
            boxes.append(f'<div class="spec-box"><div class="val">{min(ranges)}\u2013{max(ranges)} km</div><div class="lbl">Rækkevidde (WLTP, varierer pr. variant)</div></div>')
            boxes.append(f'<div class="spec-box"><div class="val">{matched_specs[0]["forbrug"]}</div><div class="lbl">Forbrug (laveste variant)</div></div>')
        else:
            boxes.append(f'<div class="spec-box"><div class="val">{s["co2_g_km"]:.0f} g/km</div><div class="lbl">CO\u2082-udledning</div></div>')
            boxes.append(f'<div class="spec-box"><div class="val">{s["forbrug"]}</div><div class="lbl">Forbrug</div></div>')
            boxes.append(f'<div class="spec-box"><div class="val">{fmt_kr(s["ejerafgift_halvaar_kr"])}</div><div class="lbl">Halvårlig ejerafgift</div></div>')
    ...
    if all_grouped:
        ...
        if suggestion:
            ...
            compare_html = (
                ...
                f'samme biltype, tæt på i pris.</p>'
...
<link href="https://fonts.googleapis.com/css2?family=Readex+Pro:wght@400;500;600;700&family=Inter:wght@400;500;600&family=JetBrains+Mono:wght@400;500;700&display=swap" rel="stylesheet">
...
<main>
  <div class="wrap">
    <p class="breadcrumb"><a href="carforyou.html">CarForYou</a> / <a href="modeller.html">Modeller</a> / {maerke} {model}</p>
    ...
    <div class="cta-box">
      ...
      <a href="carforyou.html#compare">Se fuld sammenligning \u2192</a>
    </div>
  </div>
</main>
{FOOTER}
...
<main>
  <div class="wrap">
    <p class="breadcrumb"><a href="carforyou.html">CarForYou</a> / Modeller</p>
...
"""
    with open("/home/claude/modeller.html", "w", encoding="utf-8") as f:
        f.write(html)
...
def main():
    ...
    for (maerke, model), records in sorted(grouped.items()):
        ...
        with open(f"/home/claude/{slug}", "w", encoding="utf-8") as f:
            f.write(html)
    ...
    with open("/home/claude/model-index.json", "w", encoding="utf-8") as f:
        json.dump(index_entries, f, ensure_ascii=False, indent=2)

    # Also emit as JS (same pattern as data.js/sponsorships.js/articles.js) so
    # client-side pages like sammenlign.html can use it without a fetch() call,
    # which avoids the file:// CORS issue discussed earlier in this project.
    with open("/home/claude/model-index.js", "w", encoding="utf-8") as f:
    ...
    # Specs keyed by "maerke|model" -- currently only populated for VW, since
    # that's the only brand we've verified real spec data for (see
    # parse_vw_specs.py). Other brands are simply absent from this object;
    # pages check for that and show the honest "not yet available" note
    # rather than a fabricated number.
    spec_summary = {}
    for (maerke, model), records in grouped.items():
        matched = [VW_SPECS[(model, r["variant"])] for r in records if (model, r["variant"]) in VW_SPECS]
        if not matched:
            continue
        key = f"{maerke}|{model}"
        if matched[0]["type"] == "electric":
            ranges = [s["raekkevidde_km"] for s in matched if s["type"] == "electric"]
            spec_summary[key] = {
                "type": "electric",
                "raekkevidde_km_min": min(ranges),
                "raekkevidde_km_max": max(ranges),
                "forbrug": matched[0]["forbrug"],
            }
        else:
            spec_summary[key] = {
                "type": "petrol",
                "co2_g_km": matched[0]["co2_g_km"],
                "forbrug": matched[0]["forbrug"],
                "ejerafgift_halvaar_kr": matched[0]["ejerafgift_halvaar_kr"],
            }
    with open("/home/claude/model-specs.js", "w", encoding="utf-8") as f:
        f.write("// Auto-generated by build_model_pages.py -- do not hand-edit.\n")
        f.write("// Keyed by \"Maerke|Model\". Only brands with verified real spec data\n")
        f.write("// appear here -- see parse_vw_specs.py. Absence is intentional, not a bug.\n")
        f.write("const MODEL_SPECS = ")
        f.write(json.dumps(spec_summary, ensure_ascii=False, indent=2))
        f.write(";\n")
