...
# PATCH: the original script rebuilt VW_SPECS (per-variant) from a raw
# vw_master_full.txt fetch + parse_vw_specs.py, neither of which survived
# the export from the shared chat. We already have the aggregated output
# (model-specs.js, keyed by "Maerke|Model") from that same pipeline run,
# so we load it directly instead of re-deriving it. This changes nothing
# about what ends up on the pages -- it's the same numbers, already computed.
with open("/home/claude/model-specs-existing.js", encoding="utf-8") as f:
    specs_text = f.read()
...
"""
    with open("/home/claude/work/project/generated/modeller.html", "w", encoding="utf-8") as f:
        f.write(html)
...
def main():
    ...
    for (maerke, model), records in sorted(grouped.items()):
        ...
        with open(f"/home/claude/work/project/generated/{slug}", "w", encoding="utf-8") as f:
            f.write(html)
    ...
    with open("/home/claude/work/project/generated/model-index.json", "w", encoding="utf-8") as f:
        json.dump(index_entries, f, ensure_ascii=False, indent=2)

    with open("/home/claude/work/project/generated/model-index.js", "w", encoding="utf-8") as f:
